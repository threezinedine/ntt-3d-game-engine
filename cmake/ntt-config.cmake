...
macro(ntt_configure_imgui)
    if (NTT_USE_GLFW)
        set(IMGUI_BACKEND_GLFW TRUE)
    else()
        if (NTT_PLATFORM_WINDOWS)
            set(IMGUI_BACKEND_WIN32 TRUE)
        elseif (NTT_PLATFORM_LINUX)
            set(IMGUI_BACKEND_X11 TRUE)
        else()
            message(FATAL_ERROR "Unsupported platform for ImGui backend")
        endif()
    endif()

    if (NTT_USE_GRAPHICS_OPENGL)
        set(IMGUI_RENDERER_OPENGL TRUE)
    elseif (NTT_USE_GRAPHICS_VULKAN)
        set(IMGUI_RENDERER_VULKAN TRUE)
    else()
        message(FATAL_ERROR "No graphics API selected for ImGui renderer")
    endif()

    ntt_find_package(ntt-imgui "Dependencies")
endmacro()
